  ...
  useEffect(() => {
    axios.get("/user").then((res) => {
      ...
      clone.map(
        (cl, index) =>
          cl.createAt =
            ((Number(Date.now()) - Number(Date.parse(res.data[index].createAt))) / (3600*1000*24)).toFixed(2)
      );
      setUsers(clone);
    });
  }, []);
      ...
      buttons: [
        {
          ...
          onClick: () => {
            if (id)
              axios.post(`/user/lock/${id}`).then((res) => {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            if (id)
              axios.post(`/user/unlock/${id}`).then((res) => {
