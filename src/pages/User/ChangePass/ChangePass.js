...
function ChangePass(props) {
  ...
  function onSubmit(e) {
    e.preventDefault();
    if (
      state.comfirmPassword !== state.newPassword ||
      state.comfirmPassword.length < 6 ||
  ...
  return (
    <div
      class="user-changepass"
      style={{ backgroundImage: "url(../city.jpg)" }}
    >
      <br />
      <ToastContainer />
      <div
        className="card change-pass col-sm-5"
        style={{ marginTop: "100px", marginBottom: "70px", opacity: 0.81 }}
      >
        ...
        <form style={{ margin: "20px" }} onSubmit={onSubmit}>
          ...
          {err && (
            <FormHelperText id="component-error-text" style={{ color: "red" }}>
              mật khẩu mới không khớp hoặc ít hơn 6 kí tự
            </FormHelperText>
          )}
